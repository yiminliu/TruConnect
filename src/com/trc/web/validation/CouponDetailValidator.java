...
public class CouponDetailValidator implements Validator {
  ...
  public void validateForCreateCoupon(Object target, Errors errors) {
    ...
    if (!errors.hasErrors()) {
      ...
      checkEndDateForCreateCoupon(coupon.getEndDate(), errors);
      checkQuantity(coupon, errors);
    }
  }
  ...
  private void checkStartDateForCreateCoupon(Date startDate, Errors errors) {
    if (startDate != null && startDate.compareTo(new Date()) < 0) {
      errors.rejectValue("startDate", "coupon.date.notActive", "Not a valid coupon start date.");
    }
  }

  private void checkEndDateForCreateCoupon(Date endDate, Errors errors) {
    if (endDate != null && endDate.compareTo(new Date()) <= 0) {
      errors.rejectValue("endDate", "coupon.date.expired", "Not a valid coupon end date.");
    }
  }
